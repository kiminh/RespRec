...
class DrData(object):
  def __init__(self, inputs, outputs, weights, u_inputs, i_inputs):
    assert inputs.shape[1] == u_inputs.shape[1] + i_inputs.shape[1]

    self._inputs = inputs
    self._outputs = outputs
    self._weights = weights
    self._u_inputs = u_inputs
    self._i_inputs = i_inputs
    self._nnz_input = inputs.shape[1]
    self._tot_input = inputs.max() + 1
    self._data_size = outputs.shape[0]
    self._n_user = u_inputs.shape[0]
    self._n_item = i_inputs.shape[0]
    self._start = 0
  ...
  def next_sample(self, batch_size):
    users = np.random.choice(self.n_user, batch_size)
    items = np.random.choice(self.n_item, batch_size)
    _inputs = []
    for user, item in zip(users, items):
      _input = np.concatenate([self.u_inputs[user], self.i_inputs[item]])
...
def get_dr_data(tf_flags):
  def _get_dr_data(file_base):
    data_file = file_base + '.dta'
    data_df = pd.read_csv(data_file, sep='\t', header=None)
    inputs = data_df.values[:, i_input].astype(int)
    outputs = data_df.values[:, -1].astype(float)
    ...
    data_dir = path.dirname(file_base)
    user_file = path.join(data_dir, 'user.ft')
    user_df = pd.read_csv(user_file, sep='\t', header=None)
    u_inputs = user_df.values[:, i_u_input].astype(int)
    item_file = path.join(data_dir, 'item.ft')
    item_df = pd.read_csv(item_file, sep='\t', header=None)
    i_inputs = item_df.values[:, i_i_input].astype(int)

    data_set = DrData(inputs, outputs, weights, u_inputs, i_inputs)
    return data_set

  data_dir = tf_flags.data_dir
  i_input = parse_index(tf_flags.i_input)
  i_u_input = parse_index(tf_flags.i_u_input)
  i_i_input = parse_index(tf_flags.i_i_input)

  train_base = path.join(data_dir, 'train')
  valid_base = path.join(data_dir, 'valid')
  test_base = path.join(data_dir, 'test')
  train_set = _get_dr_data(train_base)
  valid_set = _get_dr_data(valid_base)
  test_set = _get_dr_data(test_base)
  return train_set, valid_set, test_set
